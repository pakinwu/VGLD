...
def main(args):
    ...
    while cur_epoch < args.epochs:
        ...
        for idx, batch in enumerate(train_loader_0):
            ...
            total_loss = loss_kitti + 0.1 * loss_kitti_domain + args.beta * loss_kitti_ss
            total_loss.backward()
            optimizer.step()
            ...
            if global_step % args.eval_freq == 0:
                ...
                with torch.no_grad():
                    ...
                    if kitti_save:
                        model_save_name = f'/model_none_{kitti_ckpt_save_name}_step{global_step}.pth'
                        torch.save(model.state_dict(), args.log_dir + model_save_name)
                model.train()
            if global_step % 200 == 0:
                print(f"epoch{cur_epoch} : {global_step} / {num_total_steps} ; kitti_Loss {loss_kitti} ")
            global_step += 1
        cur_epoch += 1
...
if __name__ == '__main__':
    ...
    # Model
    parser.add_argument('-td', '--train_dataset', type=List[str], default=['kitti'], help='dataset to train on')
    parser.add_argument('-vd', '--val_dataset',   type=List[str], default=['kitti'], help='dataset to validate on')
    ...
    if args.exp_name is None:
        ...
        args.exp_name = f"k_ss_{args.rde_model}_lr{lr}_bz{args.batch_size}_{args.fusion}"
