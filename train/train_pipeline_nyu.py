...
def main(args):
    ...
    while cur_epoch < args.epochs:
        ...
        for idx, batch in enumerate(train_loader_0):
            ...
            total_loss = loss_nyu + 0.1 * loss_nyu_domain + args.beta * loss_nyu_ss
            total_loss.backward()
            optimizer.step()
            ...
            if global_step % args.eval_freq == 0:
                ...
                with torch.no_grad():
                    ...
                    if nyu_save :
                        model_save_name = f'/model_{nyu_ckpt_save_name}_none_step{global_step}.pth'
                        torch.save(model.state_dict(), args.log_dir + model_save_name)
                model.train()
            if global_step % 200 == 0:
                print(f"epoch{cur_epoch} : {global_step} / {num_total_steps} nyu_Loss {loss_nyu} ; ")
            global_step += 1
        cur_epoch += 1
...
if __name__ == '__main__':
    ...
    # Model
    parser.add_argument('-td', '--train_dataset', type=List[str], default=['nyu'], help='dataset to train on')
    parser.add_argument('-vd', '--val_dataset',   type=List[str], default=['nyu'], help='dataset to validate on')
    ...
    if args.exp_name is None:
        ...
        args.exp_name = f"n_ss_{args.rde_model}_lr{lr}_bz{args.batch_size}_{args.fusion}"
